...
- **Piper** binds to `127.0.0.1:{PIPER["internal_port"]}`. Pure-Python via `piper-tts[http]` in a uv venv, runs upstream's `python -m piper.http_server`. `PIPER["voices"]` is a list of slugs (`<lang>-<voice>-<quality>`); all live in the same `/Users/Shared/piper-voices/` dir, daemon serves all of them, and clients pick at request time via `"voice": "<slug>"` in the POST body. 40+ languages supported upstream.
