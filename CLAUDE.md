...
Secrets come from the `mini` 1Password vault via the `op` CLI (desktop-app
integration). The first `op` call in a deploy triggers Touch ID — no session
env var to set. Needed whenever a service's `require_api_key` flag is True or
`BESZEL["enabled"]` is True; a deploy that touches no secrets never calls `op`.
...
| `vault.py` | 1Password CLI helpers — secrets fetched at deploy time |
...
- **ComfyUI** binds to `127.0.0.1:{COMFYUI["internal_port"]}`. Opinionated stack: Flux.1 Kontext [dev] GGUF for edit-by-reference img2img + Flux.1 Fill [dev] GGUF for masked inpaint/outpaint + Z-Image Turbo GGUF for pure txt2img, all on MPS (FP8 unsupported on the MPS backend), weights pulled from HuggingFace by `tasks/comfyui.py`. Only one diffusion model is resident per request — workflow selects which. Z-Image txt2img replaces Ollama's experimental MLX imagegen runner, which panics on z-image-turbo / flux2-klein (ollama/ollama#16079, still open as of v0.24); ComfyUI runs the same model cleanly. Z-Image loads via the same city96 `UnetLoaderGGUF` / `CLIPLoaderGGUF` nodes; its Qwen3-4B encoder auto-detects to the z_image text encoder (CLIPLoader type left non-flux), and it reuses the Flux `ae.safetensors` VAE — no new VAE download. PyTorch nightly overrides the stable torch from `requirements.txt`. Memory: ~14 GB resident; coexisting with Gemma 26B (~18 GB) overflows 24 GB — calling app evicts ollama (`"keep_alive": 0`) before invoking ComfyUI. Workflow JSONs under `files/comfyui-workflows/` sync to the install dir.
...
- **Beszel agent** (optional, `BESZEL["enabled"]`) — native LaunchDaemon dialing the raspi hub via WebSocket using `BESZEL["hub_url"]` + a TOKEN/KEY pair from 1Password. No inbound port; nothing for pf to allow.
...
**Do NOT read secret values into your context.** All live credentials are in `/etc/secrets/*` on the Mini (env files written by `tasks/secrets.py`) and in the `mini` 1Password vault. `group_data/all.py` itself contains only non-secret config — it is safe to read and to edit when mirroring additions made to `group_data/all.example.py`.
...
- Reading raw values from `op item get ...` / `op read ...` (item names, field *labels*, and `op item list`/membership checks are fine — values are not)
...
Public keys for the deploy/admin user are listed in `SSH["authorized_keys"]` in `group_data/all.py`. They're managed idempotently — added if missing, never removed by IaC. Private keys are stored in 1Password (SSH-key item type) for the user's personal use; the deploy never reads them.

## 1Password

Items live in a 1Password vault named `mini`. See `vault.py` docstring for the full item list. Secrets are fetched locally at deploy time via the `op` CLI (desktop-app integration — first call triggers Touch ID, no session env var) and written to `/etc/secrets/` on the Mini (never committed to git). `op` is only called when a service flag enables auth.
...
openssl rand -hex 32
# paste it into the `api_key` hidden field on the matching 1Password item:
...
- Flux Kontext Q6_K (+ T5 Q5_K_M + CLIP-L + VAE + LoRA): ~14 GB resident when loaded. Flux Fill Q6_K is a drop-in alternative (~9.86 GB diffusion weight vs Kontext's 9.17 GB → ~14.7 GB total resident); only one diffusion model is loaded per request, so on-disk coexistence has no RAM cost.
- Z-Image Turbo Q8_0 (~6.5 GB) + Qwen3-4B encoder Q5_K_XL (~3 GB) + reused Flux VAE: ~11 GB peak resident — the lightest of the three diffusion workflows and the fastest (8 steps, distilled). The all-GGUF path was chosen over the official bf16 weights specifically for 24 GB: bf16 (`z_image_turbo_bf16` ~12 GB + `qwen_3_4b` ~8 GB) peaks ~20 GB, too tight here. If outputs ever show colour/decode artifacts, the Flux VAE may diverge from Z-Image's own `ae.safetensors` — swap to Z-Image's copy then.
